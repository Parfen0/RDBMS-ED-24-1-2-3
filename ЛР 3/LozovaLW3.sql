...
CREATE TABLE Students (
    StudentID INT PRIMARY KEY, -- Ідентифікатор студента, ціле число
	FirstName NVARCHAR(50),    -- Ім'я студента, рядок
	LastName NVARCHAR(50),     -- Прізвище студента, рядок
	EnrollmentDate DATE        -- Дата запису, дата 
);

CREATE TABLE Courses (
    CourseID INT PRIMARY KEY, -- Ідентифікатор курсу, ціле число
	CourseName NVARCHAR(50),  -- Ім'я студента, рядок
	LastName NVARCHAR(50),    -- Прізвище студента, рядок
    EnrollmentDate DATE       -- Дата запису, дата
);

INSERT INTO Students (StudentID, FirstName, LastName, EnrollmentDate)
VALUES (1, 'Andrew', 'Aaron', '2000-05-15'),
       (2, 'Alison', 'Renee', '2001-03-22'),
	   (3, 'Neil', 'Kevin', '1999-10-10');

INSERT INTO Courses (CourseID, CourseName, LastName, EnrollmentDate)
VALUES (101, 'Mathmatics', 'Basic algebra and calculus', '2000-05-15'),
       (102, 'Physics', 'Introduction to mechanics', '2001-03-22'),
       (103, 'Computer Science', 'Programming fundaments','1999-10-10');
...
ALTER TABLE Courses ADD Email NVARCHAR(100);
...
CREATE TABLE SALES (
    SALES_ORDER_ID BIGINT NOT NULL, --Унікальний ідентифікатор замовлення
	REVISION_NUMBER TINYINT NOT NULL, --Інкремент для відстеження змін
    ...
    ONLINE_ORDER_FLAG TINYINT NOT NULL CHECK (ONLINE_ORDER_FLAG IN (0, 1)), -- Замовлення створене онлайн чи менеджером
    CUSTOMER_ID BIGINT NOT NULL, -- Унікальний ідентифікатор клієнта
    SALES_PERSON_ID BIGINT, -- Унікальний ідентифікатор співробітника
    TERRITORY_ID BIGINT, -- Унікальний ідентифікатор території
    SUB_TOTAL DECIMAL(18,4) NOT NULL, -- Загальна сума замовлення
    TAX_AMT DECIMAL(18,4), -- Загальна сума податків
    FREIGHT DECIMAL(18,4), -- Сума доставки
    MODIFIED_DATE DATETIME NOT NULL -- Дата і час останньої зміни
);

INSERT INTO SALES (
    SALES_ORDER_ID, REVISION_NUMBER, ORDER_DATE, DUE_DATE, SHIP_DATE, ONLINE_ORDER_FLAG,
